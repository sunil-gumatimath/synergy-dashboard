...
const DocumentList = ({ employeeId, documents, onDocumentAdded, onDocumentDeleted, isLoading }) => {
                        ...
                        >
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h4 className="font-medium text-main truncate">{document.name}</h4>
                                    <div className="flex items-center gap-3 text-xs text-muted mt-1">
                                        <span className={`px-2 py-0.5 rounded border ${getDocumentTypeColor(document.type)}`}>
                                            {document.type}
                                        </span>
                                    ...
                                    {document.notes && (
                                        <p className="text-xs text-muted mt-1 italic">{document.notes}</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-1 flex-shrink-0">
                                ...
                                >
                                    {downloadingId === document.id ? (
                                        ...
                                        <Download size={16} className="text-muted hover:text-main" />
                                    )}
                                </button>
                                <button
                                    className="p-2 hover:bg-tertiary rounded transition-colors"
                                ...
                                >
                                    <Trash size={16} className="text-muted hover:text-danger-color" />
                                </button>
                            </div>
                        </div>
                    ))}
