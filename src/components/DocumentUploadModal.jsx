...
const DocumentUploadModal = ({ isOpen, employeeId, onClose, onDocumentUploaded }) => {
    ...
    const validateFile = (file) => {
        ...
        if (!ALLOWED_FILE_TYPES.includes(file.type)) {
            newErrors.file = 'Invalid file type. Allowed types: PDF, DOCX, DOC, JPG, PNG';
            return newErrors;
        }
    ...
    const handleSubmit = async (e) => {
        ...
        const { data, error } = await documentService.upload(employeeId, selectedFile, formData);
        ...
        setTimeout(() => {
            ...
            if (error) {
                setErrors({ submit: error.message || 'Failed to upload document. Please try again.' });
                setUploadProgress(0);
            } else {
            ...
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="modal-body">
                        ...
                        {/* File Drop Zone */}
                        <div
                            className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors ${selectedFile ? 'border-primary-color bg-primary-color/5' : 'border-border-color hover:border-primary-color'
                        ...
                        >
                            ...
                            {selectedFile ? (
                                <div>
                                    <FileText size={48} className="mx-auto mb-3 text-primary-color" />
                                    <p className="text-main font-medium">{selectedFile.name}</p>
                                    <p className="text-sm text-muted mt-1">
                                        {documentService.formatFileSize(selectedFile.size)}
                                    </p>
                                    <p className="text-xs text-muted mt-2">Click to change file</p>
                                </div>
                            ) : (
                                <div>
                                    <Upload size={48} className="mx-auto mb-3 text-muted" />
                                    <p className="text-main font-medium">Drop file here or click to browse</p>
                        ...
                        {errors.file && <span className="error-message mt-2 block">{errors.file}</span>}
