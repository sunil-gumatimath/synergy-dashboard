-- ============================================
-- AURORA EMS - FINAL COMPLETE DATABASE SETUP
-- Version: 5.0 | Date: 2024-12-07
-- All Features with Comprehensive Seed Data
-- ============================================
...
-- ============================================
-- SECTION 7: SEED DATA - 50 EMPLOYEES
-- ============================================
...
-- ============================================
-- SECTION 8: SEED DATA - TASKS (25 Tasks)
-- ============================================
INSERT INTO public.tasks (title, description, status, priority, due_date, tags) VALUES
    ...
    ('Expense Approval Workflow', 'Create automated expense approval workflow', 'To Do', 'Medium', CURRENT_DATE + 16, ARRAY['expense', 'workflow', 'automation']),
    ('Asset Tracking System', 'Implement barcode-based asset tracking', 'In Progress', 'Medium', CURRENT_DATE + 22, ARRAY['assets', 'tracking', 'inventory']),
    ('Performance Review Templates', 'Create customizable performance review templates', 'To Do', 'Low', CURRENT_DATE + 28, ARRAY['hr', 'performance', 'templates']),
    ('Payroll Automation', 'Automate payroll calculation and disbursement', 'In Progress', 'High', CURRENT_DATE + 12, ARRAY['payroll', 'automation', 'finance']),
    ('Training Certificate Generator', 'Auto-generate certificates for completed trainings', 'Review', 'Low', CURRENT_DATE + 6, ARRAY['training', 'certificates', 'automation']);
...
INSERT INTO public.calendar_events (title, description, date, time, end_time, type, location, is_all_day, color, recurrence) VALUES
    ...
    ('Investor Meeting', 'Quarterly investor update presentation', CURRENT_DATE + 25, '10:00', '11:30', 'meeting', 'Board Room', FALSE, '#dc2626', 'none'),
    ('HR Policy Review', 'Annual HR policy review and updates', CURRENT_DATE + 30, '14:00', '16:00', 'meeting', 'HR Conference Room', FALSE, '#7c3aed', 'none');
...
INSERT INTO public.support_tickets (title, description, category, priority, status) VALUES
    ...
    ('Software license expired', 'Adobe Creative Cloud license expired, need renewal', 'it_equipment', 'high', 'open'),
    ('Network connectivity issues', 'Intermittent WiFi disconnections in 3rd floor', 'technical', 'medium', 'in_progress'),
    ('Asset return request', 'Need to return old monitor and keyboard', 'general', 'low', 'open');
...
INSERT INTO public.trainings (title, description, category, instructor, duration_hours, start_date, end_date, max_participants, location, is_mandatory, status) VALUES
    ...
    ('Cybersecurity Awareness', 'Security best practices and threat prevention', 'compliance', 'IT Security Team', 6, CURRENT_DATE + 12, CURRENT_DATE + 12, 40, 'Virtual', TRUE, 'upcoming'),
    ('Time Management', 'Productivity and time management techniques', 'soft-skills', 'External Trainer', 4, CURRENT_DATE + 18, CURRENT_DATE + 18, 25, 'Training Room A', FALSE, 'upcoming');
...
INSERT INTO public.announcements (title, content, type, priority, is_pinned, target_departments, published_at, expires_at) VALUES
    ...
    ('New Parking Policy', 'Updated parking policy effective from next month. Please register your vehicle with security.', 'info', 'normal', FALSE, NULL, NOW(), NOW() + INTERVAL '45 days'),
    ('Team Outing - Save the Date', 'Annual company outing planned for March 2025. More details coming soon!', 'event', 'normal', FALSE, NULL, NOW(), NOW() + INTERVAL '120 days');
...
INSERT INTO public.onboarding_workflows (name, description, department, is_active) VALUES
    ...
    ('Finance Onboarding', 'Onboarding workflow for finance team', 'Finance', TRUE);

-- ============================================
-- SECTION 14: SEED DATA - PERFORMANCE REVIEWS
-- ============================================
...
SELECT 
    ...
    8,
    10,
    ARRAY['Strong technical skills', 'Good team collaboration', 'Meets deadlines consistently'],
    ARRAY['Could improve documentation', 'Needs to work on public speaking'],
    'Overall excellent performance with room for growth in communication skills',
    'completed',
    NOW() - INTERVAL '30 days'
FROM public.employees e
JOIN public.employees m ON e.manager = m.name
WHERE e.role = 'Employee' AND e.status = 'Active'
LIMIT 10;

-- ============================================
-- SECTION 15: SEED DATA - PAYROLL RECORDS
-- ============================================
...
SELECT 
    ...
    salary,
    '{"hra": 15000, "transport": 3000, "meal": 2000}'::jsonb,
    '{"tax": 8000, "pf": 5000, "insurance": 2000}'::jsonb,
    0,
    5000,
    salary + 20000,
    salary + 5000,
...
-- ============================================
-- SECTION 16: SEED DATA - EXPENSES
-- ============================================
INSERT INTO public.expenses (employee_id, category, amount, description, expense_date, status, approved_at)
SELECT 
    id,
    (ARRAY['Travel', 'Meals', 'Office Supplies', 'Software', 'Training'])[FLOOR(RANDOM() * 5 + 1)],
    FLOOR(RANDOM() * 5000 + 500),
    'Business expense for project work',
...
-- ============================================
-- SECTION 17: SEED DATA - ASSETS
-- ============================================
INSERT INTO public.assets (name, asset_tag, category, description, purchase_date, purchase_cost, current_value, status, assigned_to, location, warranty_expiry)
VALUES
    ('MacBook Pro 16"', 'ASSET-001', 'laptop', 'M2 Pro, 32GB RAM, 1TB SSD', '2023-06-15', 250000, 200000, 'assigned', (SELECT id FROM public.employees WHERE email = 'admin@gmail.com'), 'Bengaluru Office', '2026-06-15'),
    ('Dell Monitor 27"', 'ASSET-002', 'monitor', '4K UHD Display', '2023-05-20', 35000, 28000, 'assigned', (SELECT id FROM public.employees WHERE email = 'rajesh.kumar@aurora.com'), 'Delhi Office', '2026-05-20'),
    ('iPhone 14 Pro', 'ASSET-003', 'mobile', 'Company phone for managers', '2023-09-10', 120000, 90000, 'assigned', (SELECT id FROM public.employees WHERE email = 'priya.sharma@aurora.com'), 'Mumbai Office', '2024-09-10'),
    ('HP Printer', 'ASSET-004', 'printer', 'LaserJet Pro M404dn', '2023-03-15', 25000, 20000, 'available', NULL, 'Bengaluru Office', '2026-03-15'),
    ('Logitech Webcam', 'ASSET-005', 'webcam', '4K Pro Webcam', '2023-07-20', 15000, 12000, 'assigned', (SELECT id FROM public.employees WHERE email = 'ananya.iyer@aurora.com'), 'Chennai Office', '2025-07-20'),
    ('Standing Desk', 'ASSET-006', 'furniture', 'Electric height-adjustable desk', '2023-04-10', 45000, 40000, 'assigned', (SELECT id FROM public.employees WHERE email = 'vikram.reddy@aurora.com'), 'Hyderabad Office', '2028-04-10'),
    ('iPad Air', 'ASSET-007', 'tablet', 'Design team tablet', '2023-08-05', 55000, 45000, 'assigned', (SELECT id FROM public.employees WHERE email = 'rohit.verma@aurora.com'), 'Pune Office', '2024-08-05'),
    ('Dell Laptop', 'ASSET-008', 'laptop', 'XPS 15, 16GB RAM, 512GB SSD', '2024-01-15', 120000, 110000, 'assigned', (SELECT id FROM public.employees WHERE email = 'arjun.nair@aurora.com'), 'Kochi Office', '2027-01-15'),
    ('Conference Phone', 'ASSET-009', 'phone', 'Polycom conference system', '2023-02-20', 35000, 30000, 'available', NULL, 'Conference Room A', '2026-02-20'),
    ('Ergonomic Chair', 'ASSET-010', 'furniture', 'Herman Miller Aeron', '2023-05-10', 85000, 75000, 'assigned', (SELECT id FROM public.employees WHERE email = 'sanjay.joshi@aurora.com'), 'Mumbai Office', '2033-05-10');

-- ============================================
-- SECTION 18: SEED DATA - TIME ENTRIES
-- ============================================
INSERT INTO public.time_entries (employee_id, date, clock_in, clock_out, break_minutes, status)
SELECT 
    id,
    CURRENT_DATE - FLOOR(RANDOM() * 7)::INTEGER,
    (CURRENT_DATE - FLOOR(RANDOM() * 7)::INTEGER + TIME '09:00:00')::TIMESTAMPTZ,
    (CURRENT_DATE - FLOOR(RANDOM() * 7)::INTEGER + TIME '18:00:00')::TIMESTAMPTZ,
    60,
    'completed'
FROM public.employees
WHERE status = 'Active' AND employment_type = 'Full-time'
...
-- ============================================
-- AURORA EMS v5.0 - DATABASE SETUP COMPLETE!
-- ============================================
-- 
-- Summary:
-- ✓ 26 Tables created
...
-- ✓ 25 Tasks seeded
-- ✓ 16 Calendar events seeded
-- ✓ 15 Support tickets seeded
-- ✓ 10 Trainings seeded
-- ✓ 10 Announcements seeded
-- ✓ 5 Onboarding workflows seeded
-- ✓ 10 Performance reviews seeded
-- ✓ 20 Payroll records seeded
-- ✓ 25 Expense records seeded
-- ✓ 10 Assets seeded
-- ✓ 30 Time entries seeded
